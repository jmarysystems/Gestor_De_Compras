 ...
 * @author ana
 */
public class Tabela_Pesquisa_BD_Produto_Por_Fornecedor extends javax.swing.JPanel {
   
    private Produto_Por_Fornecedor_Home          Produto_Por_Fornecedor_Home;
    private Menu_Pesquisa_Produto_Por_Fornecedor Menu_Pesquisa;
        
    private List<produto_por_fornecedor.Produtoporfornecedor> list;
    ...
    public Tabela_Pesquisa_BD_Produto_Por_Fornecedor( Produto_Por_Fornecedor_Home Produto_Por_Fornecedor_Home2 ) {
    ...
    public void tabela(Menu_Pesquisa_Produto_Por_Fornecedor Menu_Pesquisa2){ 
    ...
    private void tabelaInicio(){
        new Thread() {   @Override public void run() { try { Thread.sleep( 1 ); 
            ...
            for (int i = 0; i < list.size(); i++){
                ...
                q.setParameter( 1, list.get(i).getIdFornecedor() );
                
                List<produto_por_fornecedor.Produtoporfornecedor> list2 = q.getResultList();
                mostrarLista( list2 );
                
                Menu_Pesquisa.tfPesquisa.setText( list.get(i).getNomeourazaosocial() );
                
                q = JPAUtil.em().createNativeQuery( "SELECT * FROM SCHEMAJMARY.FORNECEDOR WHERE ID = ?", Fornecedor.class );
                q.setParameter( 1, list.get(i).getIdFornecedor() );
                
                List<fornecedor.Fornecedor> list3 = q.getResultList();
                Menu_Pesquisa.setFornecedor( list3.get(i) );
                break;
            }
    ...
    public void mostrarLista( List<produto_por_fornecedor.Produtoporfornecedor> listR ){  try {                    
            ...
            if ( listR.isEmpty() ){ //JOptionPane.showMessageDialog( null , "Nenhum dado encontrado!"); 
                ...
                for (int i = 0; i < listR.size(); i++){
                    ...
                    Tabela_Pesquisa_BD_Produto_Por_Fornecedor_SysPdv t1 = new Tabela_Pesquisa_BD_Produto_Por_Fornecedor_SysPdv( this, i, listR.get(i).getCodigoproduto() );        
                    t1.setName("Tabela_Pesquisa_Venda_SysPdv");         
                    t1.start(); 
                  /*
                    new Thread() {   @Override public void run() { try { Thread.sleep( 1 ); 
                        tmPesquisa.setValueAt( vendaEmXDias( listR2.get(i2).getCodigoproduto(), 7  ) , i, 7);
                        tmPesquisa.setValueAt( vendaEmXDias( listR2.get(i2).getCodigoproduto(), 30 ) , i, 8);
    ...
    private void tbPesquisaLinhaSelecionada() { 
        try{
            ...
            if ( tbPesquisa.getSelectedRow() != -1){
                ...
                if( Usuario_Permissoes.booAlterarprodutoporfornecedor(Usuario_Logado.ID ) == true ) { Menu_Pesquisa.btAlterar.setEnabled(true);  }

                if( Usuario_Permissoes.booExcluirprodutoporfornecedor( Usuario_Logado.ID ) == true ) { Menu_Pesquisa.btExcluir    .setEnabled(true); }
                Menu_Pesquisa.btVisualizar.setEnabled(true);
                
                Menu_Pesquisa.btGestor.setEnabled(true);
                Menu_Pesquisa.btPrint.setEnabled(true);
            } else{
                ...
                Menu_Pesquisa.btGestor.setEnabled(false);
                Menu_Pesquisa.btPrint.setEnabled(false);
            }
        } catch( Exception e ) {}
    }
